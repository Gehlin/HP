...
describe('buildSession', () => {
  it('builds a session with empty answers, a generated id, and a startTime', async () => {
    const s = await buildSession(['xyz-001'], null, true, 'drill')
  ...
  it('sets mode to untimed and clears timeLimitSeconds when timeLimitSeconds is null', async () => {
    const s = await buildSession(['xyz-001'], null, true, 'drill')
    expect(s.mode).toBe('untimed')
    expect(s.timeLimitSeconds).toBeUndefined()
  })

  it('sets mode to timed and keeps timeLimitSeconds when a limit is given', async () => {
    const s = await buildSession(['xyz-001'], 3600, true, 'exam')
    expect(s.mode).toBe('timed')
    expect(s.timeLimitSeconds).toBe(3600)
  })

  it('passes through instantFeedback as given when studyMode is not set', async () => {
    expect((await buildSession(['xyz-001'], null, true, 'drill')).instantFeedback).toBe(true)
    expect((await buildSession(['xyz-001'], null, false, 'drill')).instantFeedback).toBe(false)
  })

  it('forces instantFeedback to true when studyMode is true, even if the caller passed false', async () => {
    const s = await buildSession(['xyz-001'], null, false, 'drill', true)
    expect(s.instantFeedback).toBe(true)
    expect(s.studyMode).toBe(true)
  })

  it('passes the type through unchanged for exam and drill', async () => {
    expect((await buildSession(['xyz-001'], null, true, 'exam')).type).toBe('exam')
    expect((await buildSession(['xyz-001'], null, true, 'drill')).type).toBe('drill')
  })

  it('returns an empty questionIds array for an empty pool', async () => {
    const s = await buildSession([], null, true, 'drill')
    expect(s.questionIds).toEqual([])
  })

  it('handles a single-item pool', async () => {
    const s = await buildSession(['xyz-001'], null, true, 'drill')
    expect(s.questionIds).toEqual(['xyz-001'])
  })

  it('keeps questions that share a passage adjacent, pulled to the position of the first occurrence', async () => {
    ...
    const s = await buildSession(input, null, true, 'drill')
    expect(s.questionIds).toEqual(['las-p01-q03', 'las-p01-q01', 'las-p01-q05', 'xyz-001'])
  })

  it('silently drops duplicate ids in the input pool (groupLinkedPassages de-dupes)', async () => {
    ...
    const s = await buildSession(['xyz-001', 'xyz-002', 'xyz-001'], null, true, 'drill')
    expect(s.questionIds).toEqual(['xyz-001', 'xyz-002'])
  })

  it('passes through unknown ids (not present in the question bank) without crashing', async () => {
    const s = await buildSession(['not-a-real-id'], null, true, 'drill')
...
describe('updateAnswer', () => {
  ...
  it('records an answer on the current session', async () => {
    saveSession(await buildSession(['xyz-001'], null, true, 'drill'))
...
describe('skipQuestion', () => {
  ...
  it('adds a question id to skipped', async () => {
    saveSession(await buildSession(['xyz-001', 'xyz-002'], null, true, 'drill'))
...
describe('toggleFlag', () => {
  it('adds a flag on first toggle and removes it on second toggle', async () => {
    saveSession(await buildSession(['xyz-001'], null, true, 'drill'))
    toggleFlag('xyz-001')
...
describe('saveQuestionTime / saveQuestionQuality', () => {
  it('merges times for multiple questions instead of overwriting', async () => {
    saveSession(await buildSession(['xyz-001', 'xyz-002'], null, true, 'drill'))
  ...
  it('merges qualities for multiple questions instead of overwriting', async () => {
    saveSession(await buildSession(['xyz-001', 'xyz-002'], null, true, 'drill'))
...
describe('finishSession', () => {
  it('is a no-op when there is no current session', async () => {
    await expect(finishSession()).resolves.not.toThrow()
    expect(loadHistory()).toEqual([])
  })

  it('sets endTime, records streakBefore, and pushes into history', async () => {
  ...
  it('records a correct SRS answer for each answered question', async () => {
  ...
  it('records a wrong SRS answer when the chosen answer does not match', async () => {
    const xyz001 = questions.find(q => q.id === 'xyz-001')!
  ...
  it('does not record SRS stats for a question that was never answered', async () => {
    saveSession(await buildSession(['xyz-001', 'xyz-002'], null, true, 'drill'))
    updateAnswer('xyz-001', 'C')
    await finishSession()
  ...
  it('updates the streak on a fresh (first-ever) session to 1', async () => {
    saveSession(await buildSession(['xyz-001'], null, true, 'drill'))
    await finishSession()
    expect(loadStats().streak).toBe(1)
  })

  it('caps stored history at 50 entries, keeping the newest first', async () => {
