...
function StepReveal({ steps, color = 'emerald' }: { steps: { label: string; content: string; result?: string }[]; color?: string }) {
  ...
  const activeColor = color === 'emerald' ? 'bg-emerald-600' : 'bg-blue-600'
  const btnColor = color === 'emerald' ? 'text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/10' : 'text-blue-400 border-blue-500/30 hover:bg-blue-500/10'
  const resultColor = color === 'emerald' ? 'text-emerald-400' : 'text-blue-400'
  return (
  ...
  return (
    <div className="min-h-screen bg-app text-[var(--color-ink)] pt-topnav pb-8">
      ...
      <div className="max-w-lg mx-auto px-4 py-10 pb-24">
        ...
        {/* Header */}
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-[11px] font-bold tracking-[0.1em] uppercase px-3 py-1.5 rounded-full mb-4">
