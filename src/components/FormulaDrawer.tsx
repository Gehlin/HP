...
export default function FormulaDrawer({ questionType, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed bottom-0 left-0 right-0 z-50 max-h-[80vh] flex flex-col bg-slate-900 border-t border-slate-700 rounded-t-2xl shadow-2xl overflow-hidden animate-slide-up">
        ...
        <div className={`shrink-0 border-b ${card.borderColor} border-opacity-40 px-5 pt-4 pb-3`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className="text-xs text-slate-400 ml-2">{card.tagline}</span>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 pb-safe">
          {card.sections.map(section => (
            <div key={section.title}>
              <div className="text-[10px] font-black tracking-widest uppercase text-slate-500 mb-2">
                {section.title}
              </div>
                ...
                {section.items.map((item, i) => (
                  <div key={i} className={`rounded-xl px-4 py-3 ${card.bgColor} border ${card.borderColor} border-opacity-30`}>
                    <div className="flex items-start gap-3">
                      <span className="text-xs text-slate-400 shrink-0 pt-0.5 min-w-[80px]">{item.label}</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-white font-mono leading-snug">
