...
- [x] Run `npm run build` to verify no errors. Also visually verify the page structure makes sense by reviewing the JSX render order: mode cards → quick CTA → quick drills → section drills → Avancerat toggle → [advanced filters hidden by default] → bottom CTA.
  <!-- Done: `npm run build` completes with zero errors. JSX render order confirmed: mode cards → quick drills → top CTA → repetition panel (conditional) → Sektionsträning grid → Avancerat toggle (mode!==repetition only) → advanced filters (advancedOpen gated) → bottom CTA. Slight ordering difference (quick drills precede top CTA rather than follow) matches task-5 implementation note and is intentional. -->
