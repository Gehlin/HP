...
- [ ] Update the values (not the names) of these drifted tokens in `src/index.css` `:root` to match the spec exactly, preserving every existing variable name so the 1000+ existing `var(--color-x)` call sites across the app keep working unchanged:
