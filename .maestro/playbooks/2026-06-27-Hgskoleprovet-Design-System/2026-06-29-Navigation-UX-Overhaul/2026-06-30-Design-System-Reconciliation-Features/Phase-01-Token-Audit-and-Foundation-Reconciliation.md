...
- [x] Run `npm run build` and confirm it exits with zero TypeScript errors. Then run `npm run dev`, open the app in a browser, and visually confirm: the page background is warm cream, body text reads as a slightly warmer near-black than before, and card borders look like soft warm hairlines rather than green-tinted edges. Take a screenshot of the Home screen for the record.

  **Completion note:** `npm run build` exits clean — zero TypeScript errors (only the existing pre-app chunk-size warning, unrelated to this work). A dev server was already running on `localhost:5173`; used a throwaway Playwright script (`node` + the project's local `playwright` devDependency) to load the page, dismiss the first-run onboarding modal ("Hoppa över"), and capture the Home screen at 1440×900. Screenshot saved to `Working/home-screen.png`. Visual confirmation: page background reads as warm cream/beige (not stark white or the old paper tone), the "God dag" heading and body copy read as a warm near-black rather than cool gray-black, and the white "Välkommen! Kom igång nu." card shows a soft warm hairline border with no green tint. Matches spec expectations from tasks 1–4.

- [x] Grep the full `src` directory for any leftover legacy color references that should now route through tokens instead: `#080C14`, `bg-slate-`, `text-slate-`, `border-slate-`, `text-blue-`, `text-violet-`, `bg-violet-`. Fix any matches found (there should be few or none, since most dark-theme cleanup already happened in earlier phases) — this task exists purely as a safety net before building further phases on top of the foundation.

  **Completion note:** The grep did **not** turn up "few or none" — it returned 161 hits across 25 files (0 for `#080C14`/`bg-slate-`/`border-slate-`, 2 for `text-slate-`, 93 for `text-blue-`, 46 for `text-violet-`, 43 for `bg-violet-`). Inspecting a sample (`NogGuide.tsx`, `KvaGuide.tsx`, `XyzGuide.tsx`, `MathGuide.tsx`, `Results.tsx`, etc.) showed the overwhelming majority is **not** stray legacy debris but a deliberate, pervasive subject-color-coding system (KVA = blue, XYZ = violet) used for badges, tabs, and accents across the quant "Guide" pages and hub/list pages app-wide. The design spec (`DESIGN-SYSTEM-01.md`, confirmed in task 1) defines no blue/violet tokens at all, so converting this system to CSS custom properties would mean inventing brand-new, unauthorized tokens and touching ~25 files' worth of subject-branding — a much bigger, design-level decision than a "safety net" sweep, and too large/risky to do correctly as one blind mechanical pass. Left untouched and flagged here for a dedicated future phase rather than half-converting it.

  Fixed the two concrete, narrowly-scoped issues this sweep was actually meant to catch:
  - `src/pages/Practice.tsx` (2 hits): replaced stray `text-slate-700` (an undertheme leftover with no warm-palette equivalent nearby) with `var(--color-ink-muted)`, matching the secondary-text convention used everywhere else in that file.
  - `src/pages/Results.tsx`: this is the exact case task 1 flagged ("`--color-border` is referenced 6 times... currently rendering as `border-color: initial`... fix together with this"). Replaced all 6 undefined `var(--color-border)` references with the established `var(--color-card-border)` hairline token, and replaced the adjacent hardcoded dark-theme active/selected-state utility classes (`border-blue-500 text-blue-400 bg-blue-500/10`, `border-red-500 text-red-400 bg-red-500/10`, `border-amber-500 text-amber-400 bg-amber-500/10`, `border-blue-600 text-blue-400 bg-blue-500/10`) on the review-filter buttons and bookmark badge with the spec-accurate semantic tokens added in task 3: `--color-selected-bg`/`--color-selected-border` (the "Alla" filter), `--color-wrong-bg`/`--color-wrong-border`/`--color-wrong-badge` (the "Fel" filter), and `--color-gold-deep`/`--color-gold-muted` (the "Markerade"/★ flagged filter and the 🔖 bookmark-saved state, both being "marking" semantics).

  Verified with `npm run build` — zero TypeScript errors.
