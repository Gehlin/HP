...
- [ ] Verify the Statistics screen: run `npm run dev`, navigate to `/progress`. Confirm segmented control switches data ranges, activity bars show correct day data, type accuracy bars use warm accent colors, HP score and XP cards are visible with no dark backgrounds.
