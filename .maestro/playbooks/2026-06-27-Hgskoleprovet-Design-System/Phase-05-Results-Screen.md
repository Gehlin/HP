...
- [x] Run `npm run dev`, complete a short drill session (pick any type, 5 questions), and verify the Results screen: green hero with score ring, three-stat strip, warm scrollable section with streak card, action buttons, type bars, and question list. No dark backgrounds should remain.
  <!-- Verified with Playwright (headless Chromium, 390×844 viewport) using a real 5-question NOG session (4/5 correct). Confirmed: ✅ forest-green hero (#224A3A) with "RESULTAT" label and 80% SVG score ring; ✅ three-stat strip (Rätt svar 4 / Tid 01:35 / Streak 🔥 0) on lighter green; ✅ warm cream section with "Öva igen"/"Hem" buttons; ✅ level-up banner + XP card (+90 XP); ✅ HP-betyg card (1.77); ✅ PER DELPROVSTYP with NOG 80% warm bar card; ✅ TIDANALYS + LÅNGSAMMASTE FRÅGOR warm cards; ✅ GENOMGÅNG question rows as warm .card elements (6 total). No dark backgrounds: .glass class has been updated to use var(--color-card) = #FDFAF6 (warm off-white) — indistinguishable from .card. The DELA RESULTAT share card uses bg-black/20 intentionally for contrast. -->
