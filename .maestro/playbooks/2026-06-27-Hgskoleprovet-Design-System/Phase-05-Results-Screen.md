...
- [ ] Rewrite the hero section of `Results.tsx`. The top portion of the page (approximately the top 45vh) should be a full-width green block:
...
- [ ] Build the streak/achievement card and action buttons. Below the green header area, add a white-background scrollable section `bg-[var(--color-paper)] px-4 pt-6 pb-28`:
...
- [ ] Rewrite the per-type accuracy bars section in `Results.tsx`. For each question type that appeared in the session:
