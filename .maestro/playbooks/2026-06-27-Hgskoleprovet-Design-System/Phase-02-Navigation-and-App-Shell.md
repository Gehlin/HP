...
- [x] Verify the navigation works: run the dev server (`npm run dev`) and confirm all 4 tabs are visible with warm paper background, tapping each tab navigates correctly, the active tab shows a green top indicator and green text, and the Profil tab opens the placeholder page with its menu rows.
  - _Note: Verified with Playwright (headless Chromium, 390×844 viewport). All 4 tabs confirmed: Hem/Öva/Statistik/Profil. Nav background `rgb(241, 236, 227)` (warm paper). Each tab routes correctly: `/`, `/practice`, `/progress`, `/profil`. Active tab (Profil) label color `rgb(34, 74, 58)` (forest green), inactive (Hem) `rgb(154, 154, 146)` (faint). `nav-pill` green top indicator confirmed on active tab. Profil page renders "Profil" heading and all 4 menu rows (Teori, Bokmärken, Inställningar, Repetitionskö) with chevrons._
