...
- [x] Update `src/App.tsx`: change `PageLoader` spinner colors from `border-slate-700 border-t-blue-400` to `border-[var(--color-paper-dark)] border-t-[var(--color-green)]`. Update the keyboard shortcut overlay modal: change `bg-slate-900 border-slate-700` to `bg-[var(--color-card)] border-[var(--color-card-border)]`, `text-slate-300` to `text-[var(--color-ink-muted)]`, `bg-slate-800 border-slate-600 text-slate-200` kbd style to `bg-[var(--color-paper-dark)] border-[var(--color-card-border)] text-[var(--color-ink)]`, `text-slate-400` to `text-[var(--color-ink-muted)]`, `text-slate-500` to `text-[var(--color-ink-faint)]`. Also update the offline banner from `bg-amber-600` to `bg-[var(--color-terracotta)]`.
  <!-- Done: Updated PageLoader spinner to var(--color-paper-dark)/var(--color-green), keyboard shortcut modal to card/ink tokens, offline banner to var(--color-terracotta), and close button to ink-faint/ink hover. -->

- [x] Run `npm run dev` from the project root to start the dev server. Verify the app loads: the background should now be warm cream (#F1ECE3), body text should be dark (#1A1A18), and headings/UI should be in Hanken Grotesk. The app will look partially unstyled (many components still use Tailwind dark classes) but the global foundation should be visible. Fix any TypeScript or CSS parse errors before continuing.
  <!-- Done: Dev server started cleanly on port 5175 with no TypeScript or CSS errors. Playwright computed-style check confirmed: body background = rgb(241,236,227) (#F1ECE3 ✅), body color = rgb(26,26,24) (#1A1A18 ✅), font = "Hanken Grotesk" ✅. CSS variables --color-paper, --color-ink, and --font-sans all resolve correctly. The onboarding modal retains dark styles (expected — component-level updates are in later phases), but the underlying page background is visibly warm cream. Zero console errors. -->
