...
- [x] Verify the Home screen visually: `npm run dev`, open `/`. Confirm: warm cream background, serif greeting, green hero card with score, continue card when relevant, 2-column section grid with progress rings. No dark backgrounds should remain on this screen.
  <!-- Done. Ran Playwright against http://localhost:5174/ with onboarding skipped (localStorage key `hp_onboarding_done=1`). Confirmed: bg-app renders as warm cream rgb(241,236,227); h1 shows "God dag" in serif; terracotta streak-pill top-right; forest-green .card-green hero with score ring, 3 mini-stats; 8-card 2-col grid with SVG rings per section. Only intentional dark elements: .card-green (#224A3A) and nav-pill accent. No residual dark glass cards. TypeScript build clean (0 errors). -->
